...
int main(int argc, char *argv[]) {
    ...
    if (argc != 11) {
        printf("./main [outfileheader] [sub_entries] [sampling_rate(Hz)] "
               "[sampling_number] [dynamic range 0(+/-25V) or 1(+/-2.5V)] [ch1 "
               "Vth(V)] [ch2 Vth(V)] [trigger source 0(ch1) or 1(ch2) or "
               "2(or)] [Trig Edge RISE=0 or FALL=1] [uri]\n");
        printf("ex) daq out 1000 1000000 1024 2 0.1 0.1 2 0 usb:1.5.5\n");
        return -1;
    }
    ...
    */
    ...
    printf(
        "TRIGGER SOURCE         : %d    [ch1=0,ch2=1,or-trig=2,and-trig=3]\n",
        trigger_source);
