...
void fitSpectrum( const std::string resultdir, const int n )
{
    ...
    TF1  *fit[2];
    for ( int i = 0; i < 2; i++ ) {
        histSp[i] = new TH1F( Form( "histSp%i", i + 1 ), Form( "histSp%i", i + 1 ), ran_bin.get( ), ran_min.get( ), ran_max.get( ) );
        if ( ch.get( ) == 1 )
            fit[i] = new TF1( Form( "fit%i", i + 1 ), "gaus", fitCh1_min.get( ), fitCh1_max.get( ) );
        else
            fit[i] = new TF1( Form( "fit%i", i + 1 ), "gaus", fitCh2_min.get( ), fitCh2_max.get( ) );
    }
    ...
    tex.DrawLatex( ran_max.get( ) * 0.55, 0.95 * histSp[0]->GetMaximum( ), Form( "mean : %4.2f +- %3.2f", mean_1, meane_1 ) );
    tex.DrawLatex( ran_max.get( ) * 0.55, 0.9 * histSp[0]->GetMaximum( ), Form( "sigma : %3.2f +- %3.2f", sigma_1, simgae_1 ) );
    ...
    tex.SetTextSize( 0.04 );
    tex.DrawLatex( ran_max.get( ) * 0.55, 0.95 * histSp[1]->GetMaximum( ), Form( "mean : %4.2f +- %3.2f", mean_2, meane_2 ) );
    tex.DrawLatex( ran_max.get( ) * 0.55, 0.9 * histSp[1]->GetMaximum( ), Form( "sigma : %3.2f +- %3.2f", sigma_2, simgae_2 ) );
